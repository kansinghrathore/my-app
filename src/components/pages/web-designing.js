...
function Webdesign(){
  const [page, setPage] = useState({}); 
      useEffect(()=>{
        let url = `${process.env.REACT_APP_API_ROOT_OPTION}/pages/2479`
    ...
    return( <>
    <SEO
          title='Website Designing | Best Web Hosting Provider in INDIA | Logic Daddy'
   ...
   <div className="WhyHosting">
      <div className="container">
         ...
         {page.data.acf.why_web_designing.heading.heading}<span> {page.data.acf.why_web_designing.heading.color_heading}</span>
            <p className="TextCenter">{page.data.acf.why_web_designing.heading.description}</p>
         </div>
         <div className="row">
         {
                  page.data.acf.why_web_designing.box.map((g)=>{
                     return (
   ...
   <div className="centersection WorkFlow">
      <div className="container">
         <h2 className="Heading" data-aos="zoom-in">{page.data.acf.our_workflow.heading}</h2>
         <div className="step-progressbar-wrap">
            <ul className="arrow-step-wrap" data-aos="zoom-in">
            {
                  page.data.acf.our_workflow.workflow.map((g)=>{
                     return (
                      <li className="step" >{g.list}</li>
                     )
                  })
               }
            </ul>
